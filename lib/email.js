...
export async function sendLoginVerificationCode({ toEmail, code }){
  const transporter = getTransporter()
  if(!transporter) return
  ...
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#0f172a;line-height:1.6;background:#f8fafc;padding:24px;">
      <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:24px;">
        <h2 style="color:#7c3aed;margin:0 0 12px;font-size:20px;">Your login verification code</h2>
        <p style="margin:0 0 12px;">Use the code below to complete your sign-in. It expires in <strong>10 minutes</strong>.</p>
        <div style="font-size:32px;font-weight:800;letter-spacing:8px;color:#0f172a;margin:16px 0;">${code}</div>
        <p style="margin:0 0 12px;color:#475569;font-size:13px;">If you did not attempt to log in, please secure your account immediately.</p>
      </div>
    </div>
  `
